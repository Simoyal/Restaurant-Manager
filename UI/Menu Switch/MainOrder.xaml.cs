...
namespace UI.Menu_Switch
{
    ...
    public partial class MainOrder : UserControl, iSwitchable
    {
        ...
        {
            if (MessageBox.Show("If you want to update client data select no", "Confirmation", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                // switching to the update order window
                Switcher.Switch(new UpdateMenuOrderWindow());
            else
                Switcher.Switch(new UpdateClient());
        }
        #endregion
