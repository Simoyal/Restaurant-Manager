...
namespace DAL
{
    ...
    public class Dal_XML_imp : idal
    {
        ...
        {
            // getting the branch
            XElement branch  = (from item in BranchRoot.Elements()
        ...
        {
            // searching
            XElement client = (from item in ClientRoot.Elements()
        ...
        /// <summary>
        /// Updating data on a client
        /// </summary>
        /// <param name="client"></param>
        public void UpdateClient(Client client)
        {
            XElement tmp = (from item in ClientRoot.Elements()
                               where item.Element("Address").Value == client.Address
                               select item).FirstOrDefault();
            // no matching
            if (tmp == null)
                throw new Exception("No matching client with this address");

            tmp = FromClientToXelement(client);
            ClientRoot.Save(ClientPath);
        }
        /// <summary>
        /// Get all the client
